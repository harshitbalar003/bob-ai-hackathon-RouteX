...
- **Unified Priority Queue:** Excursions, shipment exceptions, idle assets, and ML predictions merged into a single weighted worklist — the operator always sees the most urgent action first. Predicted-risk items are visually distinct from confirmed excursions (dashed badge, no citation).
...
- **Excursion Forecaster (ML):** `GET /api/v1/predictions/excursion-risk` — P(temperature breach within 4 hours) for in-transit cold-chain shipments. Calibrated `HistGradientBoostingClassifier` (scikit-learn). Disabled by default (`ML_ENABLED=false`); the system runs on heuristic baselines without it. The ML layer never classifies severity or assigns regulatory citations — that stays in the deterministic rule-pack engine.

---

## ⚠️ Known Limitations

- **Synthetic training data:** The ML excursion forecaster is trained exclusively on data generated by our own thermal model (`app/seed/telemetry.py`). Held-out metrics (PR-AUC 1.0 on test seeds) demonstrate a working prediction pipeline and its integration with the deterministic core — they do not reflect validated real-world accuracy. The model learns that generator, not real reefer physics. Validating it properly would require a corpus of real reefer sensor data across diverse routes, container types, and operators. See `docs/ml-models.md` for full detail.
